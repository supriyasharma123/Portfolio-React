...
const dummy = {
  path: '//*[@id="home"]/div/div/button',
  action: "Click",
  name: "ABCD",
  successMetric: true,
};
//TODO :: remove dummy/response data
...
function applyChanges(pages) {
  ...
  // pages[0].changelogs.map((changeLog) => {
  response.map((changeLog) => {
    console.log("Fpt changeLog", changeLog);
    var element = getElementByXpath('//*[@id="root"]/nav/div/a');
    console.log("Fpt element", element);
    ...
    if (changeLog.type == "editEle") {
      ...
      if (changeLog.properties.media || changeLog.properties.alt) {
        updateImage(
          element,
          changeLog.properties.media,
          changeLog.properties.alt
        );
...
function getElementByXpath(t) {
  console.log("Fpt path", t);
  ...
  try {
    ...
    console.log("Fpt res", res.singleNodeValue);
...
async function sendEvent(e) {
  try {
    fetch(
      "https://99t0hot2ah.execute-api.ap-south-1.amazonaws.com/event-handler/event",
      {
        method: "POST",
        body: JSON.stringify({
      ...
      .then((response) => response.json())
      .then((json) => console.log("Fpt event send", json, e));
  } catch (ex) {
    console.log("FPT sendEvent failed for e", e);
  }
}

async function getPagesFromResolver() {
  fetch(
    ...
    .then((json) => {
      ...
      enrichLocationDetails(res1.locationDetails);
      enrichCampaignDetails(res1.campaignDetails);
      pages = res1.experiencePages;
      successCriteria = res1.successCriteria; //dummy;
      addApplyChangesHandler();
    });
}
...
function enrichCampaignDetails(res) {
  campaignDetails.workflowId = res.workflowId;
  campaignDetails.campaignId = res.campaignId;
  campaignDetails.experienceId = res.experienceId;
  campaignDetails.ruleSetId = res.ruleSetId;
  campaignDetails.clientId = res.clientId;
  campaignDetails.url = res.url;
}
...
function invokeApplyChanges() {
  console.log("Fpt invokeApplyChanges called" , document)
  applyChanges(pages);
  if (!isRequestFromFibrEditor) {
    const e1 = {
      eventType: "PAGE_VIEW_EVENT",
      path: campaignDetails.url,
      timestamp: Date.now(),
    };
    sendEvent(e1);
    if (successCriteria) {
      ...
      if (successCriteria.action != "Click") {
        //TODO :: check for page view success criteria and test
        event.timestamp = Date.now();
        sendEvent(e2);
      } else {
        var ele = getElementByXpath(successCriteria.path);
        if (ele) {
...
function addApplyChangesHandler() {
  console.log("Fpt document.readyState", document.readyState);

  if (document.readyState == "loading" || document.readyState == "interactive" ) {
    console.log("Fpt document.readyState 1", document.readyState);
    document.addEventListener("DOMContentLoaded", () => {
      console.log("loaded")
      invokeApplyChanges()
    });
  } else {
    console.log("Fpt document.readyState 2", document.readyState);
    invokeApplyChanges();
  }
}
